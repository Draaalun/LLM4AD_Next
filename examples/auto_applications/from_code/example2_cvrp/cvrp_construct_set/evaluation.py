...
class CVRPSEvaluation(Evaluation):
    ...
    def evaluate(self, heuristic: callable, ins_to_be_evaluated_id: Set | List | None=None, training_mode=True) -> Optional[dict, list]:
        ...
        elif 'ics' in self.whocall or 'ICS' in self.whocall or self.whocall in ['dyca']:
            return {'instance_performance': instance_performance, 'list_performance':dis}
        elif self.whocall in ['eoh', 'reevo', 'funsearch']:
            return (np.mean(dis), instance_performance)
        else:
            return np.mean(dis)

"""Evaluator for the ``ml_hyperpara_benchmark`` example application.

Loads ``classifier.py`` from the algorithm worktree, checks HYPERPARAMS API, runs
``train`` / ``predict``, and aggregates RMSE for evolution.
"""
...
@BaseEvaluator.register("ml_hyperpara_benchmark_evaluator")
class MLHyperparaBenchmarkEvaluator(PythonEvaluator):
    """Evaluator for the ml_hyperpara_benchmark task.

    Evolution is intended to change only ``HYPERPARAMS`` dict
    inside the EVOLVE block; ``train`` / ``predict`` and XGBRegressor stay fixed.
    """
    ...
    def __init__(self):
        ...
        self._template_dir = Path(__file__).parent / "ml_hyperpara_algorithm"

    @property
    def name(self) -> str:
        """Get the evaluator name."""
        return "ml_hyperpara_benchmark_evaluator"
    ...
    def _load_dataset(self, data_path: str) -> tuple:
        ...
        if not data_path.endswith(".csv"):
            raise ValueError(f"Only CSV files are supported, got: {data_path}")
    ...
    async def evaluate(self, cfg: EvalContext) -> EvaluationResult:
        """Evaluate hyperparameters plus fixed XGBRegressor pipeline.
        ...
        try:
            ...
            # Check HYPERPARAMS exists in code
            if "HYPERPARAMS" not in code:
                return EvaluationResult(
                    score=0.0,
                    metrics={"score": 0.0},
                    success=False,
                    error_message="HYPERPARAMS dict not found in classifier.py",
                    duration_ms=(time.time() - start_time) * 1000,
                )
            ...
            required = ("train", "predict", "HYPERPARAMS")
            ...
            if missing:
                return EvaluationResult(
                    ...
                    metrics={"score": 0.0},
                    success=False,
                    error_message=f"Missing in classifier.py: {missing}",
                    duration_ms=(time.time() - start_time) * 1000,
                )
